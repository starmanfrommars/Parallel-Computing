...
// Process 3 recieved first element -1
// Process 0 sending first 
// Process 2 recieved first element -1
...
int main(int argc, char *argv[]) {
    ...
    if(rank==0){
        ...
        printf("Process 1 sending first \n");
        MPI_Recv(data_recv,SIZE,MPI_INT,0,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);  // These two lines are swapped to avoid deadlock
        MPI_Send(data_send,SIZE,MPI_INT,0,0,MPI_COMM_WORLD);                    // rest code is same as above one
    }
...
// Process 2 recieved first element -1
// Process 1 sending first 
// Process 0 recieved first element 1
// Process 1 recieved first element 0